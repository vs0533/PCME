...
namespace PCME.Infrastructure.Repositories
{
    public class UnitOfWork<TContext> : IRepositoryFactory, IUnitOfWork<TContext>, IUnitOfWork where TContext : DbContext
    {
        ...
        public UnitOfWork(TContext context, IMediator mediator)
        {
            ...
            System.Diagnostics.Debug.WriteLine("UnitOfWork::ctor ->" + this.GetHashCode());
        }
        ...
        public async Task<bool> SaveEntitiesAsync(bool ensureAutoHistory = false, CancellationToken cancellationToken = default(CancellationToken), params IUnitOfWork[] unitOfWorks)
        {
            if (ensureAutoHistory)
            {
                _context.EnsureAutoHistory();
        ...
        public async Task<bool> SaveEntitiesAsync(bool ensureAutoHistory = false, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (ensureAutoHistory)
            {
                _context.EnsureAutoHistory();
            ...
            await _mediator.DispatchDomainEventsAsync(_context);
            var result = await this.SaveChangesAsync(ensureAutoHistory);
            return true;
        }
    }
}
